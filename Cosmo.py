...
def alph2AS(z,ar,at):
	rd_fid = 147.78
	dis_fid = distance(.31,.69,h=.676)
	H_fid = dis_fid.Hz(z)#*100.
	DA_fid = dis_fid.da(z)
	cHi_fid = dis_fid.cHz(z)#/100.
	DV_fid=((1.+z)**2.*z*cHi_fid*DA_fid**2.)**(1/3.)
	#F_fid = (1.+z)/cHi_fid*DA_fid
	F_fid = (1.+z)*H_fid/dis_fid.c*DA_fid
	alpha_pl = ar
	alpha_perp = at
	alpha = (alpha_perp**2.*alpha_pl)**(1/3.)
